...
class OnepageController extends APIController
{
    ...
    public function storeShippingMethod()
    {
        ...
        if (
            ...
            || ! $validatedData['outlet']
            || ! Cart::saveShippingMethod($validatedData['shipping_method'], $validatedData['outlet'])
        ) {
    ...
    public function storeDeliveryDatetime()
    {
        $validatedData = $this->validate(request(), [
        ...
        //        $product_ids = Cart::getCart()->items->pluck('product_id');
        //        $product = Product::where('id', 13)->first();
        //        $processing_time = $product->getAttribute('processing_time');

        if (
            Cart::hasError()
            || ! $validatedData['delivery_date']
            || ! $validatedData['timeslot']
           /* || ! $validatedData['additional_notes'] */
            || ! Cart::saveDeliveryTimeSlot($validatedData['delivery_date'], $validatedData['timeslot'], $validatedData['additional_notes'])
        ) {
            return response()->json([
                'redirect_url' => route('shop.checkout.cart.index'),
            ], Response::HTTP_FORBIDDEN);
