<x-admin::layouts>
    @php
        ...
        $outlet = DB::table('outlets')->where('id', $outlet_id)->value('name');
    @endphp
    <!-- Page Title -->
    <x-slot:title>
        {{
            $role == 'Outlet Manager'
            ? 'Orders' . ' - ' . $outlet
            : trans('admin::app.sales.orders.index.title')
            }}
        </x-slot>
        ...
        <x-admin::datagrid :src="route('admin.sales.orders.index')" :isMultiRow="true">
            ...
            <template #header="{ columns, records, sortPage, selectAllRecords, applied, isLoading}">
                <template v-if="! isLoading">
                    <div
                        class="row grid grid-cols-[0.5fr_0.5fr_1fr] grid-rows-1 items-center px-4 py-2.5 border-b dark:border-gray-800">
                        <div
                            class="flex gap-2.5 items-center select-none"
                            v-for="(columnGroup, index) in [['increment_id', 'created_at', 'status'], ['base_grand_total', 'method', 'delivery_date'], ['full_name', 'customer_email', 'location', 'image']]"
                        >
                    ...
                    >
                        ...
                        <div class="flex gap-x-2 justify-between items-center">
                            ...
                            <a :href=`{{ route('admin.sales.orders.view', '') }}/${record.id}`>
                                <span
                                    class="icon-sort-right text-2xl ltr:ml-1 rtl:mr-1 p-1.5 cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-800 hover:rounded-md"></span>
                            </a>
                        </div>
                    </div>
                </template>
